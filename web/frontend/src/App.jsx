...
export default function App() {
  ...
  const runAnalysis = async () => {
    if (!selected) return;
    setJob(null);
    const r = await fetch(`${API}/api/analyses/${selected.name}/run`, {
      method: 'POST',
    });
    if (!r.ok) {
      setJob({
        id: 'local',
        status: 'error',
        error: `Failed to start: ${r.status} ${r.statusText}`,
        output_files: {},
      });
      return;
    }
    ...
    <div className="flex h-screen bg-gray-50 text-gray-900">
      <aside className="w-96 border-r bg-white flex flex-col">
        <div className="p-4 border-b">
          <h1 className="font-bold text-xl">Prediction Market Analysis</h1>
          <p className="text-xs text-gray-500 mt-1">{analyses.length} analyses available</p>
          <input
            value={filter}
            onChange={(e) => setFilter(e.target.value)}
            placeholder="Filter analyses..."
            className="mt-3 w-full px-3 py-1.5 border rounded text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
        </div>
        <ul className="overflow-y-auto flex-1">
          ...
          {filtered.length === 0 && (
            <li className="p-4 text-sm text-gray-500">No analyses match "{filter}"</li>
          )}
        </ul>
      </aside>
        ...
        {!selected && (
          <div className="h-full flex items-center justify-center text-gray-500">
            <div className="text-center">
              <div className="text-lg">Select an analysis to begin</div>
              <div className="text-sm mt-2">Choose from the {analyses.length} analyses on the left.</div>
            </div>
          </div>
        )}
          ...
          <div className="p-8 max-w-5xl">
            ...
            <div className="mt-6">
              <button
                onClick={runAnalysis}
                disabled={isRunning}
                className="px-5 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed font-medium"
              >
                ...
                <div className="flex items-center gap-3 mb-3">
                  ...
                  {job.duration_seconds != null && (
                    <span className="text-xs text-gray-500">
                      took {fmtDuration(job.duration_seconds)}
                    </span>
                  )}
                </div>
