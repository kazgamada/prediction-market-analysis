"""Decision Dashboard (MOCKUP) — single viewport, 12 tiles, no-scroll layout."""
...
st.markdown("# Dashboard　<small style='font-size:0.7rem;color:#888;'>MOCKUP — tile タイトルクリックで詳細</small>",
            unsafe_allow_html=True)
...
with r1[0], st.container(border=True):
    tile_header("Wallet equity (30d)", "pages/3_Watchlist.py", "👛")
...
with r1[1], st.container(border=True):
    tile_header("Drawdown", "pages/7_Strategy.py", "📉")
...
with r2[0], st.container(border=True):
    tile_header("市場×戦略 matrix", "pages/7_Strategy.py", "🧪")
...
with r2[1], st.container(border=True):
    tile_header("シグナル時間帯", "pages/0_Status.py", "⏰")
...
with r2[2], st.container(border=True):
    tile_header("DD gauge (今日)", "pages/6_Execution.py", "🛑")
...
with r2[3], st.container(border=True):
    tile_header("Latency (signal→fill)", "pages/6_Execution.py", "⚡")
...
with r3[0], st.container(border=True):
    tile_header("Position exposure", "pages/6_Execution.py", "💰")
...
with r3[1], st.container(border=True):
    tile_header("Watchlist Top 5", "pages/3_Watchlist.py", "📋")
    df = pd.DataFrame({
...
with r3[2], st.container(border=True):
    tile_header("受信シグナル", "pages/6_Execution.py", "📨")
...
with r3[3], st.container(border=True):
    tile_header("Indexer lag", "pages/0_Status.py", "📡")
