...
| E-6 | E 資金安全 | P1 | kill switch/halt のレース: risk 評価→claim(PENDING→EXECUTING)→SKIP の順序で、tick 内のリスク変化が無視され、halt 中に claim された行が EXECUTING で滞留（孤児化） | `execution/executor.py:199-209` | tick 内で claim 後に再評価 or claim 前に halt 確定（要設計判断） | 報告（要判断） |
| E-7 | E 資金安全 | P1 | backfill が失敗チャンクを dead-letter に逃がしつつカーソルを単調前進。dead-letter retry が失敗するとコピー元トレードを永久取りこぼし | `indexer/backfill.py:65-118` `indexer/cursor.py:46` | 最古の未解決失敗ブロック未満にカーソルを留める or 失敗時はカーソル前進を保留（要設計判断） | 報告（要判断） |
...
- **P1**: E-1（修正済）/ F-2（修正済）/ F-1（修正済）/ **E-6 kill switch レース（要判断）** / **E-7 backfill 取りこぼし（要判断）** / B-1・B-2（要確認）。
- **P2**: E-2/E-3/E-4（修正済）/ E-8 Position side / E-9 jobs lease / CI で DB 付きテスト常時化。
- **P3**: D-1 / E-10 FK / F-3。

> **重要**: E-6/E-7 は資金安全に直結する P1 だが、修正には実行レイヤ/インデクサのアーキ判断
> （tick 内再評価の設計、カーソル前進ポリシー）を伴うため、本監査では**証拠付きで報告し、
> 修正方針は提示するに留めた**（推測で実装しない原則）。E-5 のみ contained かつ回帰テスト可能と
> 判断して修正・green 化した。
...
4. **査読への反証（pushback）**: 査読は残高ガード（`evaluator.py:184`）を P1 としたが、
   `usdc_balance_cache` は balance hook 未実装ゆえ常時 0 であり、`>0` ガードを今「修正」すると
   paper モードで risk が**常時 halt** してしまう。現状は「未実装機能の正しい no-op」であり
   P3（D-1 に統合）が妥当と判断した。フック実装（PR #4）時に 0=未取得/枯渇 を区別して是正する。
