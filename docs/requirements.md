...
## 4.7 フォーム保存の信頼性・エラー可視化（AUDIT.md 共通機能要件）

AUDIT.md に追加された「フォーム保存の信頼性/エラー可視化」を本スタック向けに対応。
Next.js の `readErrorMessage(res)` は、Streamlit では例外を捕捉して実メッセージを赤表示する
共通ヘルパー `web/forms.py::try_save()` に対応づけた。

- `try_save(fn, success_msg, toast=)`: 保存失敗時に **`st.error` で実際の例外型+メッセージ**を
  赤表示（固定文言で原因を隠さない）、成功時のみ `st.success`/トースト（緑）。無言の握りつぶし禁止。
- 適用済みハンドラ: `admin_ai_settings`（APIキー保存/削除・モデル選択）、`settings_notifications`
  （通知設定保存）、`admin_users`（ロール更新・凍結）、`admin_email`（一斉送信）。
- 既に実エラーを出していた箇所: `settings_billing` / `admin_billing`（Stripe エラーを赤表示）は現状維持。

### GAP 分析（AUDIT.md 新セクション D）

| 項目 | 着手前 | 実装後 | 工数 |
|---|---|---|---|
| 保存失敗時の実エラー表示（readErrorMessage 相当） | 固定文言/無処理で例外は Streamlit 既定の赤トレース | `try_save` で実例外を赤表示に統一 | 0.25d |
| 成否による色の出し分け（失敗=赤・成功=緑） | 一部のみ | 全対象ハンドラで統一 | 0.1d |
| 無言 return / catch 握りつぶし | notifications 等で try/except 無し | `try_save` で必ず可視化 | 0.1d |
| migration ↔ 本番スキーマ同期（Supabase bootstrap.sql 相当） | 該当（本スタックは alembic） | 起動時 `run_migrations()`（advisory lock + self-heal）で本番へ自動適用済 → 準拠 | 0d |
| 管理者表示の本番前提（OWNER_EMAIL/service_admins 相当） | OAuth のみ昇格 | `ADMIN_EMAILS`（既定 kazgamada@gmail.com）で全ログイン経路を admin 昇格（§4.6） | 0d |

> bootstrap.sql 同期は本スタックでは不要。alembic マイグレーションは各ランタイム起動時に
> `db/engine.py::run_migrations()` が advisory lock 付きで head まで適用する設計のため、
> 「本番にテーブルが無い → 保存が 500」は構造的に発生しない。
