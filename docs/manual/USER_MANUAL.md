...
| 確認するもの (Home の見る場所) | 何を意味するか | OK の基準 | NG だったら |
|---|---|---|---|
| **累積 PnL** (左上の数字) | これまでの累計損益 (米ドル建て) | 緑色 (プラス) のまま | 赤字なら §6.2 「ドローダウン拡大」 |
| **勝率** (左上の数字) | 過去のクローズした取引のうち利益が出た割合 | **55% 以上** | 50% 切ったら §6.5 「勝率低下」 |
| **最大 DD** (左上の数字) | 過去 30 日で最高益から最大いくら下落したか (%) | **-10% 以下** (= -10% より浅い、例 -5% なら OK) | -15% 超えそうなら §6.2 |
| **USDC 残高** (左上の数字) | 取引用ステーブルコイン残高 (発注の元手) | **$500 以上** | $500 切ったら §6.6 「残高不足」 |
| **今日 PnL** (左上の数字) | 今日 0:00 UTC からの損益 | 緑 or 軽微なマイナス | **-3% で警戒、-5% で自動停止** |
| **「Indexer lag」タイル** (右下) | データ取得の遅れ秒数 (リアルタイム性) | グラフが赤線 (120 秒) を越えていない | §6.4 「データ更新が止まった」 |
| **「DD gauge」タイル** (中段右) | 今日のドローダウンメーター (緑→黄→赤) | **緑ゾーン** (DD 3% 以下) | 黄なら警戒、赤なら **手動停止** 検討 |
| **「受信シグナル」タイル** (下段中央) | コピー元 wallet の最近の動き 5 件と処理結果 | **✅ (約定成功)** が多数 | **❌ (発注失敗) や ⏭ (上限スキップ) ばかり** なら §6.3 |

**用語確認:**
- **PnL** = 損益 / **DD** = ドローダウン (最高益からの下落%) / **halt** = 自動停止
- **緑** = プラス利益 / **赤** = マイナス損失
- **タイル** = Home 画面に並んだ小さなグラフ枠

**「30 秒目視」の進め方:**
1. 上の段 (KPI 6 個) を左から右へなめる → 緑の数字が並んでいれば OK
2. 12 タイルをざっと見て、**赤や黄色**が出ているタイルだけ詳しく確認
3. 怪しい数字が無ければ画面を閉じて終了。1 つでも気になるなら該当 §6.x へ
